...
export function cleanAndParse(messy: string): string {
  ...
  messy = messy.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
  ...
  function parseValue(): any {
    ...
    try {
      ...
      console.warn(
        `Parse error at ${tk.line}:${tk.col}: ${e instanceof Error ? e.message : String(e)}`,
      );
  ...
  function parseObject(): any {
    ...
    while (peek() !== 'RBRACE' && peek() !== 'EOF') {
      ...
      if (peek() === 'COLON') advance();
        ...
        console.warn(`Missing ':' after key at ${tk.line}:${tk.col}`);
      }
