...
async function installSunaForNewUser(userId: string) {
  try {
    ...
    if (!adminApiKey) {
      console.error(
        'KORTIX_ADMIN_API_KEY not configured - cannot install Suna agent',
      );
      return;
    }

    const response = await fetch(
      `${backendUrl}/admin/suna-agents/install-user/${userId}`,
      {
        method: 'POST',
        headers: {
...
export async function checkAndInstallSunaAgent(
  userId: string,
