...
export const useCustomMCPToolsData = (agentId: string, mcpConfig: any) => {
  ...
  const { data, isLoading, error, refetch } = useQuery<CustomMCPToolsResponse>({
    ...
    queryFn: async () => {
      const response = await backendApi.get(
        `/agents/${agentId}/custom-mcp-tools`,
        {
          headers: {
            ...
            ...(mcpConfig.headers
              ? { 'X-MCP-Headers': JSON.stringify(mcpConfig.headers) }
              : {}),
          },
        },
      );
      if (!response.success) {
        throw new Error(
          response.error?.message || 'Failed to fetch custom MCP tools',
        );
      }
  ...
  const updateToolsMutation = useMutation({
    mutationFn: async (enabledTools: string[]) => {
      ...
      if (!response.success) {
        throw new Error(
          response.error?.message || 'Failed to update custom MCP tools',
        );
      }
      return response.data;
    },
