...
export const AgentMCPConfiguration: React.FC<AgentMCPConfigurationProps> = ({
  ...
  const allMCPs = [
    ...
    ...(customMCPs || []).map((customMcp) => {
      if (
        customMcp.type === 'composio' ||
        customMcp.customType === 'composio'
      ) {
        return {
          name: customMcp.name,
          qualifiedName:
            ...
            `composio.${customMcp.toolkit_slug || customMcp.config?.toolkit_slug || customMcp.name.toLowerCase()}`,
          mcp_qualified_name:
            customMcp.mcp_qualified_name ||
            customMcp.config?.mcp_qualified_name,
          ...
          toolkit_slug:
            customMcp.toolkit_slug || customMcp.config?.toolkit_slug, // Add for logo system
        };
      }
      ...
      const displayType =
        customMcp.type === 'sse'
          ? 'http'
          : customMcp.type || customMcp.customType;
  ...
  const handleConfigurationChange = (mcps: any[]) => {
    ...
    console.log(
      '[AgentMCPConfiguration] Sending to parent - configured:',
      configured,
      'custom:',
      custom,
    );
