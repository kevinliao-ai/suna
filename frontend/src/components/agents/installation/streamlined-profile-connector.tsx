...
export const ProfileConnector: React.FC<ProfileConnectorProps> = ({
  ...
  const { data: composioProfiles } = useCredentialProfilesForMcp(
    composioQualifiedName,
  );
  ...
  const hasConnectedComposioProfile =
    composioProfiles && composioProfiles.length > 0;
  ...
  const mockComposioApp = useMemo(() => {
    const actualToolkit = composioToolkits?.toolkits?.find(
      (t) => t.slug === step.app_slug,
    );
    return (
      actualToolkit || {
