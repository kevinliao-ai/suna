...
import {
  ...
  ComposioTriggerType,
} from '@/hooks/react-query/composio/use-composio-triggers';
...
const ProgressStepper = ({
  ...
  currentStep: 'apps' | 'triggers' | 'config';
}) => {
  const steps = [
  ...
  return (
    <div className="space-y-3">
      {Object.entries(properties).map(([key, prop]: [string, any]) => {
        ...
        const examples: any[] = Array.isArray(prop.examples)
          ? prop.examples
          : [];
          ...
          <div className="space-y-1" key={key}>
            ...
            {type === 'number' || type === 'integer' ? (
              <Input
                ...
                inputMode="numeric"
                onChange={(e) =>
                  handle(e.target.value === '' ? '' : Number(e.target.value))
                }
...
export const EventBasedTriggerDialog: React.FC<
  EventBasedTriggerDialogProps
> = ({ open, onOpenChange, agentId, onTriggerCreated }) => {
  ...
  const [selectedTrigger, setSelectedTrigger] =
    useState<ComposioTriggerType | null>(null);
  ...
  const { data: appsData, isLoading: loadingApps } =
    useComposioAppsWithTriggers();
  const { data: triggersData, isLoading: loadingTriggers } =
    useComposioAppTriggers(selectedApp?.slug, !!selectedApp);
  const {
    data: profiles,
    isLoading: loadingProfiles,
    refetch: refetchProfiles,
  } = useComposioProfiles(
    selectedApp?.slug ? { toolkit_slug: selectedApp.slug } : undefined,
  );
  const { data: allProfiles } = useComposioProfiles(); // Get all profiles for connection status
  const { data: toolkitDetails } = useComposioToolkitDetails(
    selectedApp?.slug || '',
    { enabled: !!selectedApp },
  );
  ...
  const { variableSpecs, templateText } = useMemo(() => {
    if (!selectedWorkflow)
      return { variableSpecs: [] as any[], templateText: '' };
    const stepsAny = ((selectedWorkflow as any)?.steps as any[]) || [];
    const start = stepsAny.find(
      (s: any) =>
        s?.name === 'Start' &&
        s?.description === 'Click to add steps or use the Add Node button',
    );
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const base: any = {
        ...
        name:
          name ||
          `${selectedTrigger.toolkit.name} → ${executionType === 'agent' ? 'Agent' : 'Workflow'}`,
                  ...
                  <div className="p-6 border-b flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Choose an app to monitor for events and trigger your
                        agent
                      </p>
                    </div>
                      ...
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {search
                            ...
                            : 'No apps with triggers available'}
                        </p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {apps.map((app) => {
                          const connectionStatus = getAppConnectionStatus(
                            app.slug,
                          );
                          return (
                            <AppCard
                ...
                <div className="h-full flex flex-col">
                  ...
                  >
                    <div className="max-w-2xl mx-auto space-y-6">
                      ...
                      {!loadingProfiles &&
                      (profiles || []).filter((p) => p.is_connected).length ===
                        0 ? (
                        <div className="text-center py-8">
                          ...
                          <p className="text-sm text-muted-foreground mb-4">
                            Connect {selectedApp?.name} first to create
                            triggers.
                          </p>
                          <Button
                            variant="outline"
                        ...
                        <div className="space-y-6">
                          <div className="border rounded-lg p-4 space-y-4">
                            ...
                            <DynamicConfigForm
                              schema={selectedTrigger.config as any}
                              value={config}
                              onChange={setConfig}
                            />
                          </div>
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="space-y-2">
                                ...
                                <RadioGroup
                                  value={executionType}
                                  onValueChange={(v) =>
                                    setExecutionType(v as 'agent' | 'workflow')
                                  }
                                >
                                  <div className="flex items-center space-x-2">
                                    <RadioGroupItem
                              ...
                              {executionType === 'agent' ? (
                                ...
                                <div className="space-y-4">
                                  ...
                                  {selectedWorkflowId &&
                                  variableSpecs &&
                                  (variableSpecs as any[]).length > 0 ? (
                                    <div className="space-y-3">
                                      {(variableSpecs as any[]).map(
                                        (v: any) => (
                                          <div
                                            key={v.key}
                                            className="space-y-1"
                                    ...
                                    selectedWorkflowId && (
                                      <div className="space-y-2">
                                        ...
                                        <Textarea
                                          rows={3}
                                          value={
                                            (workflowInput?.prompt as string) ||
                                            ''
                                          }
                                          onChange={(e) =>
                                            setWorkflowInput((prev) => ({
