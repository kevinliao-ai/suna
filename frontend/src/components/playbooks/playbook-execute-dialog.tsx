...
export const PlaybookExecuteDialog: React.FC<PlaybookExecuteDialogProps> = ({
  ...
  const isValid = useMemo(() => {
    return variableSpecs.every(
      (v) =>
        !v.required || (values[v.key] !== undefined && values[v.key] !== ''),
    );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Your playbook will run in a separate thread so you can keep working
            here.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm">
                  ...
                  <div className="text-amber-800/90">
                    This playbook is currently "{playbook.status}" and can’t be
                    run. Activate it to enable execution.
                  </div>
                </div>
                <Button
                  size="sm"
            ...
            <div className="rounded-2xl border p-4 bg-muted/30">
              <div className="space-y-2">
                ...
                <div className="text-sm text-muted-foreground">
                  {playbook?.name} is processing in the background. You can
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const projectId = thread.project_id || 'default';
                        router.push(
                          `/projects/${projectId}/thread/${startedInfo.threadId}`,
                        );
                      } catch {
                        toast.error('Failed to open thread');
                      }
            ...
            <>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <Button
                  type="button"
                  onClick={handleRun}
                  disabled={executeMutation.isPending || !isValid || !isActive}
                >
