...
const isStreamableTool = (toolName: string) => {
  return Object.values(STREAMABLE_TOOLS).some((toolSet) =>
...
export const ShowToolStream: React.FC<ShowToolStreamProps> = ({
  ...
  const cleanXMLContent = (
    ...
    const parameterMatches = cleaned.match(
      /<parameter\s+name=["'][^"']*["']>([\\s\\S]*?)(<\/parameter>|$)/gi,
    );
    if (parameterMatches) {
      const paramEntries = parameterMatches
        .map((match) => {
          const fullMatch = match.match(
            /<parameter\s+name=["']([^"']*)["']>([\\s\\S]*?)(<\/parameter>|$)/i,
          );
          if (fullMatch) {
  ...
  const streamingContent = React.useMemo(() => {
    ...
    if (STREAMABLE_TOOLS.FILE_OPERATIONS.has(toolName || '')) {
      ...
      if (paramName) {
        const newMatch = content.match(
          new RegExp(
            `<parameter\\s+name=["']${paramName}["']>([\\s\\S]*)`,
            'i',
          ),
        );
        ...
        if (isEditFile) {
          ...
          if (oldMatch && oldMatch[1]) {
            const cleanContent = oldMatch[1].replace(
              /<\/code_edit>[\s\S]*$/,
              '',
            );
            return { html: cleanContent, plainText: cleanContent };
    ...
    if (STREAMABLE_TOOLS.COMMAND_TOOLS.has(toolName || '')) {
      const commandMatch = content.match(
        /<parameter\s+name=["']command["']>([\\s\\S]*?)(<\/parameter>|$)/i,
      );
    ...
    if (STREAMABLE_TOOLS.BROWSER_TOOLS.has(toolName || '')) {
      const urlMatch = content.match(
        /<parameter\s+name=["']url["']>([\\s\\S]*?)(<\/parameter>|$)/i,
      );
      const actionMatch = content.match(
        /<parameter\s+name=["']action["']>([\\s\\S]*?)(<\/parameter>|$)/i,
      );
      const instructionMatch = content.match(
        /<parameter\s+name=["']instruction["']>([\\s\\S]*?)(<\/parameter>|$)/i,
      );

      if (urlMatch && urlMatch[1]) {
        const value = urlMatch[1].trim();
        return {
          html: `<strong>url:</strong> ${value}`,
          plainText: `url: ${value}`,
        };
      }
      if (actionMatch && actionMatch[1]) {
        ...
        return {
          html: `<strong>action:</strong> ${value}`,
      ...
      if (instructionMatch && instructionMatch[1]) {
        ...
        return {
          html: `<strong>instruction:</strong> ${value}`,
          plainText: `instruction: ${value}`,
        };
      }
    }
    ...
    if (STREAMABLE_TOOLS.WEB_TOOLS.has(toolName || '')) {
      const queryMatch = content.match(
        /<parameter\s+name=["']query["']>([\\s\\S]*?)(<\/parameter>|$)/i,
      );
      const urlMatch = content.match(
        /<parameter\s+name=["']url["']>([\\s\\S]*?)(<\/parameter>|$)/i,
      );
      ...
      if (queryMatch && queryMatch[1]) {
        ...
        return {
          html: `<strong>query:</strong> ${value}`,
          plainText: `query: ${value}`,
        };
      }
      if (urlMatch && urlMatch[1]) {
        const value = urlMatch[1].trim();
    ...
    if (STREAMABLE_TOOLS.OTHER_STREAMABLE.has(toolName || '')) {
      ...
      for (const param of commonParams) {
        const match = content.match(
          new RegExp(
            `<parameter\\s+name=["']${param}["']>([\\s\\S]*?)(<\\/parameter>|$)`,
            'i',
          ),
        );
        if (match && match[1]) {
          ...
          return {
            html: `<strong>${param}:</strong> ${value}`,
  ...
  if (showExpanded && isToolStreamable) {
    return (
      <div className="my-1">
        ...
        <div
          className={`border border-neutral-200 dark:border-neutral-700/50 rounded-2xl overflow-hidden transition-all duration-500 ease-in-out transform-gpu ${
            shouldShowContent
              ? 'bg-zinc-100 dark:bg-neutral-900'
              : 'bg-muted scale-95 opacity-80'
          }`}
        >
          {/* Tool name header */}
          <button
          ...
          <div
            className={`transition-all duration-500 ease-in-out overflow-hidden transform-gpu ${
              shouldShowContent
                ? 'max-h-[350px] border-t border-neutral-200 dark:border-neutral-700/50 opacity-100'
                : 'max-h-0 border-t-0 opacity-0 scale-y-95'
            }`}
          >
            <div className="relative">
              <div
                ...
                className={`max-h-[300px] overflow-y-auto scrollbar-none text-xs text-foreground transition-all duration-400 ease-in-out transform-gpu ${
                  STREAMABLE_TOOLS.FILE_OPERATIONS.has(toolName || '') ||
                  STREAMABLE_TOOLS.COMMAND_TOOLS.has(toolName || '')
                    ? 'font-mono whitespace-pre-wrap'
                    : 'whitespace-pre-wrap'
                } ${shouldShowContent ? 'opacity-100 translate-y-0 p-3' : 'opacity-0 translate-y-3 scale-95 p-0'}`}
                style={{
                  maskImage: shouldShowContent
                    ? 'linear-gradient(to bottom, transparent 0%, black 8%, black 92%, transparent 100%)'
                    : 'none',
                  WebkitMaskImage: shouldShowContent
                    ? 'linear-gradient(to bottom, transparent 0%, black 8%, black 92%, transparent 100%)'
                    : 'none',
                }}
              >
              ...
              {/* Top gradient */}
              <div
                className={`absolute top-0 left-0 right-0 h-8 pointer-events-none transition-all duration-400 ease-in-out ${
                  shouldShowContent
              ...
              {/* Bottom gradient */}
              <div
                className={`absolute bottom-0 left-0 right-0 h-8 pointer-events-none transition-all duration-400 ease-in-out ${
                  shouldShowContent
