...
export function FileAttachment({
  ...
  const shouldLoadContent =
    (isHtmlOrMd || isCsv) && (shouldShowPreview || isCsv);
  const {
    data: fileContent,
    ...
    return (
      <div
        className={cn(
          ...
          isPdf
            ? '!min-h-[200px] sm:min-h-0 sm:h-[400px] max-h-[500px] sm:!min-w-[300px]'
            : isHtmlOrMd
              ? '!min-h-[200px] sm:min-h-0 sm:h-[400px] max-h-[600px] sm:!min-w-[300px]'
              : isCsv || isXlsx
                ? 'min-h-[300px] h-full' // Let CSV and XLSX take full height
                : standalone
                  ? 'min-h-[300px] h-auto'
                  : 'h-[300px]', // Better height handling for standalone
          className,
        )}
        style={{
        ...
        <div
          ...
          style={{
            ...
            containIntrinsicSize:
              isPdf || isHtmlOrMd ? '100% 500px' : undefined,
        ...
        >
          ...
          {!hasError && (
            <>
              {isPdf &&
                (() => {
                  const pdfUrlForRender =
                    localPreviewUrl ||
                    (sandboxId ? (pdfBlobUrl ?? null) : fileUrl);
              ...
              {isXlsx &&
                (() => {
                  const xlsxUrlForRender =
                    localPreviewUrl ||
                    (sandboxId ? (xlsxBlobUrl ?? null) : fileUrl);
                ...
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-1 px-2 py-1 rounded-xl hover:bg-background/70 text-xs font-medium transition-colors">
                    <span className="truncate max-w-[100px]">
                      {xlsxSheetNames[xlsxSheetIndex] || 'Sheet 1'}
                    </span>
                    <ChevronDown size={12} />
                  </button>
                </DropdownMenuTrigger>
  ...
  if (alignRight) {
    return (
      <div className="w-full flex justify-end">
        <div className="max-w-[85%]">{fileButton}</div>
      </div>
    );
  }
