...
export function FullScreenPresentationViewer({
  ...
  const [isDownloadingGoogleSlides, setIsDownloadingGoogleSlides] =
    useState(false);
  ...
  const loadMetadata = useCallback(
    async (retryCount = 0, maxRetries = 5) => {
      ...
      try {
        ...
        console.error(
          `Error loading metadata (attempt ${retryCount + 1}):`,
          err,
        );
        ...
        setError(
          'Failed to load presentation metadata after multiple attempts',
        );
        setIsLoading(false);
  ...
  const currentSlideData = slides.find(
    (slide) => slide.number === currentSlide,
  );
      ...
      <div className="flex-1 flex items-center justify-center bg-zinc-100 dark:bg-zinc-900 p-2 min-h-0">
        {isLoading ? (
          ...
          <div className="w-full h-full flex flex-col">
            ...
            <div
              className="flex-1 bg-transparent rounded-xl overflow-hidden"
