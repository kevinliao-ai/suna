...
const extractFromNewFormat = (content: any): CheckProfileConnectionData => {
  ...
  if (
    ...
    const extractedData = {
      ...
      connections: Array.isArray(parsedOutput.connections)
        ? parsedOutput.connections
        : [],
      connection_count: parsedOutput.connection_count || 0,
      available_tools: Array.isArray(parsedOutput.available_tools)
        ? parsedOutput.available_tools
        : [],
  ...
  if ('parameters' in parsedContent && 'output' in parsedContent) {
    const extractedData = {
      ...
      connections: Array.isArray(parsedContent.output?.connections)
        ? parsedContent.output.connections
        : [],
      connection_count: parsedContent.output?.connection_count || 0,
      available_tools: Array.isArray(parsedContent.output?.available_tools)
        ? parsedContent.output.available_tools
        : [],
...
const extractFromLegacyFormat = (
  content: any,
): Omit<CheckProfileConnectionData, 'success' | 'timestamp'> => {
...
export function extractCheckProfileConnectionData(
  ...
  const combinedData = {
    ...
    connections:
      toolLegacy.connections.length > 0
        ? toolLegacy.connections
        : assistantLegacy.connections,
    connection_count:
      toolLegacy.connection_count || assistantLegacy.connection_count,
    available_tools:
      toolLegacy.available_tools.length > 0
        ? toolLegacy.available_tools
        : assistantLegacy.available_tools,
