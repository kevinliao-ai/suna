...
const FORMAT_PATTERNS = {
  ...
  // URL list patterns
  urlList: [/^https?:\/\/\S+$/m, /(https?:\/\/\S+\s*\n){2,}/],

  // Key-value patterns
  keyValue: [/^\s*\w+\s*:\s*.+$/m, /^\s*"[^"]+"\s*:\s*.+$/m],
...
export class MCPFormatDetector {
   ...
   */
  private static detectObjectFormat(
    obj: any,
    originalText?: string,
  ...
  private static isSearchResultStructure(obj: any): boolean {
    ...
    if (obj.results && Array.isArray(obj.results)) {
      return obj.results.some(
        (item: any) => item.title && (item.url || item.link),
      );
    }

    // Data array
    if (obj.data && Array.isArray(obj.data)) {
      return obj.data.some(
        (item: any) => item.title && (item.url || item.link),
      );
    }
    ...
    if (Array.isArray(obj)) {
      return obj.some((item: any) => item.title && (item.url || item.link));
    }
