...
function extractCheckCommandOutputData(
  ...
  if (toolParsed && typeof toolParsed === 'object') {
      ...
      try {
        ...
        if (contentParsed.tool_execution) {
          ...
          if (toolExecution.result && toolExecution.result.output) {
            ...
            actualIsSuccess =
              toolExecution.result.success !== undefined
    ...
    // Check for frontend_content first (this is the actual data structure)
      ...
      toolParsed.frontend_content &&
      toolParsed.frontend_content.tool_execution
    ) {
    ...
    // Fallback to content.content structure
      ...
      if (
        toolParsed.content.content &&
        typeof toolParsed.content.content === 'string'
      ) {
        try {
  ...
  if (!output && !sessionName) {
    ...
    if (assistantParsed && typeof assistantParsed === 'object') {
      ...
      if (
        assistantParsed.frontend_content &&
        assistantParsed.frontend_content.tool_execution
      ) {
      ...
      // Fallback to content.content structure
        ...
        typeof assistantParsed.content === 'object'
      ) {
        if (
          assistantParsed.content.content &&
          typeof assistantParsed.content.content === 'string'
        ) {
          try {
...
export function CheckCommandOutputToolView({
  ...
  const isSessionRunning =
    status?.includes('still running') || status?.includes('running');
        ...
        {isStreaming ? (
          ...
          <div className="flex flex-col items-center justify-center h-full py-12 px-6 bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 text-center max-w-md">
              No session name was detected. Please provide a valid session name
              to check.
            </p>
          </div>
        )}
