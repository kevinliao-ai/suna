...
const LoggedInMenu: React.FC<UnifiedConfigMenuProps> = ({
  ...
  const [execDialog, setExecDialog] = useState<{
    ...
    playbook: any | null;
    agentId: string | null;
  }>({ open: false, playbook: null, agentId: null });
  ...
  const filteredAgents = useMemo(() => {
    ...
    const selected = selectedAgentId
      ? list.find((a) => a.agent_id === selectedAgentId)
      : undefined;
    const rest = selected
      ? list.filter((a) => a.agent_id !== selectedAgentId)
      : list;
  ...
  const handleQuickAction = (
    action: 'instructions' | 'knowledge' | 'triggers',
  ) => {
    ...
    router.push(
      `/agents/config/${agentId}?tab=configuration&accordion=${action}`,
    );
    setIsOpen(false);
  };
  ...
  const displayAgent = useMemo(() => {
    const found =
      agents.find((a) => a.agent_id === selectedAgentId) || agents[0];
    return found;
  }, [agents, selectedAgentId]);

  const currentAgentIdForPlaybooks = isLoggedIn
    ? displayAgent?.agent_id || ''
    : '';
  const { data: playbooks = [], isLoading: playbooksLoading } =
    useAgentWorkflows(currentAgentIdForPlaybooks);
                        ...
                        <DropdownMenuItem
                          ...
                          onClick={(e) => {
                            ...
                            setExecDialog({
                              open: true,
                              playbook: wf,
                              agentId: currentAgentIdForPlaybooks,
                            });
                            setIsOpen(false);
                          }}
                        >
