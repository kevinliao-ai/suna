...
export interface DocsBulletItemProps
...
export interface DocsBulletsProps
...
export const DocsBulletItem = React.forwardRef<
  HTMLLIElement,
  DocsBulletItemProps
>(
  (
      ...
      >
        {DisplayIcon && (
          <div className="flex-shrink-0 mt-0.5">
            <DisplayIcon className={cn(iconSizes[size], iconColors[variant])} />
          </div>
        )}
        ...
        <div className="flex-1 space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            {title && (
              <span
                className={cn('font-medium text-foreground', titleSizes[size])}
              >
                {title}
              </span>
            )}
...
export const DocsBullets = React.forwardRef<HTMLUListElement, DocsBulletsProps>(
  (
    ...
    return (
      <Component
        ref={ref as any}
        className={cn(listStyleClass, spacingClasses[spacing], className)}
        {...props}
      >
        {enhancedChildren}
      </Component>
