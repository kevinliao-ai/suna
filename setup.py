...
def check_suna_directory():
    """Check if we're in a AniSora repository"""
    required_dirs = ['backend', 'frontend']
    required_files = ['README.md', 'docker-compose.yaml']
    
    for directory in required_dirs:
        if not os.path.isdir(directory):
            print_error(f"'{directory}' directory not found. Make sure you're in the AniSora repository root.")
            return False
    
    for file in required_files:
        if not os.path.isfile(file):
            print_error(f"'{file}' not found. Make sure you're in the AniSora repository root.")
            return False
    
    print_success("AniSora repository detected")
    return True
...
def collect_llm_api_keys():
    ...
    print_info("You need at least one LLM provider API key to use AniSora")
...
def configure_backend_env(env_vars, use_docker=True):
    ...
    env_content = """# Generated by AniSora setup script
...
def start_suna():
    """Start AniSora using Docker Compose or manual startup"""
    print_info("You can start AniSora using either Docker Compose or by manually starting the frontend, backend and worker.")

    print(f"\n{Colors.CYAN}How would you like to start AniSora?{Colors.ENDC}")
    ...
    if use_docker:
        print_info("Starting AniSora with Docker Compose...")
...
def final_instructions(use_docker=True, env_vars=None):
    ...
    print(f"\n{Colors.GREEN}{Colors.BOLD}✨ AniSora Setup Complete! ✨{Colors.ENDC}\n")
    ...
    if env_vars and 'llm' in env_vars and 'MODEL_TO_USE' in env_vars['llm']:
        ...
        print_info(f"AniSora is configured to use {Colors.GREEN}{default_model}{Colors.ENDC} as the default LLM model")
    
    if use_docker:
        print_info("Your AniSora instance is now running!")
        print_info("Access it at: http://localhost:3000")
        print_info("Create an account using Supabase authentication to start using AniSora")
        print("\nUseful Docker commands:")
        print(f"{Colors.CYAN}  docker compose ps{Colors.ENDC}         - Check the status of AniSora services")
        ...
        print(f"{Colors.CYAN}  docker compose down{Colors.ENDC}       - Stop AniSora services")
        print(f"{Colors.CYAN}  docker compose up -d{Colors.ENDC}      - Start AniSora services (after they've been stopped)")
    else:
        print_info("AniSora setup is complete but services are not running yet.")
        print_info("To start AniSora, you need to:")
        ...
        print_info("4. Once all services are running, access AniSora at: http://localhost:3000")
        print_info("5. Create an account using Supabase authentication to start using AniSora")
...
def main():
    ...
    # Print banner
    print_banner()
    print("This wizard will guide you through setting up AniSora, an open-source generalist AI agent.\n")
    ...
    # Check if we're in the AniSora repository
    if not check_suna_directory():
        print_error("This setup script must be run from the AniSora repository root directory.")
    ...
    # Install dependencies before starting AniSora
    ...
    # Now ask how to start AniSora
    print_step(current_step, total_steps, "Starting AniSora")
